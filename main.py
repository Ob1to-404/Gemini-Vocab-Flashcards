    ...
    for category in CATEGORIES:
        ...
        if st.button(category, use_container_width=True, type=("primary" if is_selected else "secondary")):
            if category != st.session_state.current_category:
                ...
                st.session_state.word_index = 0 
                st.session_state.show_translation = False
                st.rerun() # Yangi Streamlit versiyasi uchun to'g'rilangan
...
with col2:
    ...
    if not current_word_list:
        ...
        st.markdown("---")
        # st.subheader("Bu so'zni bilasizmi? (Tanlang va keyingi so'zga o'ting)") # BU QATOR OLIB TASHLANDI
        
        # Bilaman / Bilmayman tugmalari endi sarlavhasiz
